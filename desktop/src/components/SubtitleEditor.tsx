...
export default function SubtitleEditor({ transcript, setTranscript, videoSrc, autoSave = true, onSave, file }: SubtitleEditorProps) {
	...
	const {
		...
		canUndo,
		canRedo,
	} = useHistoryState<Segment[]>(transcript.segments)
		...
		<EditorContext.Provider value={{ segments, updateSegments, videoRef }}>
			<div className="flex flex-col gap-2">
				...
				<div className="flex items-center gap-2">
					...
					<button className="btn btn-sm" onMouseDown={undo} disabled={!canUndo}>
						Undo
					</button>
					<button className="btn btn-sm" onMouseDown={redo} disabled={!canRedo}>
						Redo
					</button>
				</div>
				<div className="flex items-center gap-2">
